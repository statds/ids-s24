...
### Seaborn and Histogram and Density Plots
```{python}
plt.figure(figsize=(10,8))
sns.histplot(df['Borough'], bins=10, kde = True)
```

KDE (Kernel Density Estimate Line) is kernel density estimate line by smoothing sample points into a continuous curve and using a continuous probability density curve in one or more dimensions. 
...
plt.figure(figsize = (10, 8))
sns.regplot(x='Weight', y='Height', data=df2)
```

#### Bar plot, Box Plot and Histogram
