...
### Uconn Dining Hall Example

Let's say we want to conduct a study on foods offered at the UConn dining halls. One way to do this, would be to go online, look at the menus, and then manually chart them. But this is time consuming, and furthermore the menus are only available for about a week in advance, meaning that to to a long term study you would need to do this at least once a week, for every single dining hall. Web scraping provides an easier way to do this.

Let's first start by gathering a list of the URLs for the dining hall menus. These can be found as links on the uconn dining homepage, under "Menus & Nutrition". While it would be more practical 

### Downsides

For as powerful as webscraping is, it has it's limitations. Not every website wants you to scrape everything on it, and if they find you using a web crawler or scraper they can block your IP. Web scraping can also be slow when working with a large number of webpages, due to time spent conncting to the internet, and dosen't always work as expected with dynamically updated data.
