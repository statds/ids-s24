...
```{python}
sub_df = sub_df.rename(columns={"BusinessTravel": "Freq. Travel", 
...
sub_df["Freq. Travel"] = sub_df["Freq. Travel"].map({"Travel_Rarely": False, 
"Travel_Frequently": True, 'Non-Travel': True})

sub_df["Married"] = sub_df["Married"].map({"Single": False, "Married": True, 
"Divorced": False})
...
The following code takes the example dataset and attempts to use a random 
forest model to predict whether or not a student is likely to dine out based 
on the features of:
