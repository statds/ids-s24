...
```{python}
month_count_df = pd.crosstab(df['Created Month'], columns = 'count')

model = sm.OLS(month_count_df['count'], sm.add_constant(month_count_df['Created Month']))
# results = model.fit()
# print(results.summary())
# Fits a linear regression model between count and year, with a constant term
```

This will give us a summary of the regression coefficients including their 
standard error, t-statistics, and p-values. The output also includes
...
Let's demonstrate with an example using the Generalized Least Squares (GLS) 
estimator:
```{python}
...
We can use GLM when our response variable isn't continuous or when it's        
binomially distributed. Using our NYC rodent dataset, we

#### Time Series Analysis

```{python}
pd.crosstab(df['Created Month'], columns = 'count')
```
