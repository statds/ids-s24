...
Firstly, ensure `scipy.stats`
is installed by using `pip install scipy.stats` or   
`pip3 install scipy.stats`, depending on what version of python you are using. 
...
```{python}
result_2 = scipy.stats.ttest_ind(mhtn_means, queens_means, equal_var = True, 
                                                            alternative = 'less') 
...
distribution, with $1$ being a perfect match. [2]   
...
distribution, which are $0$ and $3$, respectively. [4]
...
variable are mutually exclusive, and observations are independent. [3]
...
different variables, categorical ones in particular. For example, we can create
a frequency table showing the relative frequencies of rodent sighting per Borough, 
where the rodent sightings are split into categories by their descriptor. 
```{python}
...
```  
At an alpha level of $0.05$, the p-value allows us to reject the null hypothesis
and conclude that the `Borough` and `Descriptor` columns are indeed dependent. 

We can further explore this data by now choosing the Borough with the most rodent
sightings and testing whether the season in which the request was created is independent
of the type of sighting, using the `Descriptor` column. 
```{python}
...
```{python}
results_4 = scipy.stats.chi2_contingency(freq_table_2)

print('test statistic:', results_4.statistic)
print('p-value:', results_4.pvalue)
print('degrees of freedom:', results_4.dof) 
``` 
At an alpha level of $0.05$, the p-value allows us to reject the null hypothesis
...
**Assumptions:** Observations are independen and ordinal.  
...
normal, so the statisic can be measured as $z = \frac{U-\mu_{U}}{\sigma_{U}}$. [5]
...
**Assumptions:** Observations are independen, ordinal, and the samples are paired.
...
results_5 = scipy.stats.mannwhitneyu(df_2022, df_2023, nan_policy = 'omit', 
                                                      alternative = 'greater')

# perform test with H_0 df_2022 < df_2023
results_6 = scipy.stats.mannwhitneyu(df_2022, df_2023, nan_policy = 'omit', 
                                                        alternative = 'less')

print('test statistic:', results_5.statistic)
print('p-value:', results_5.pvalue)
print()
print('test statistic:', results_6.statistic)
print('p-value:', results_6.pvalue)
``` 
...
essentially a comparison of their medians. 

## References 
1. [`scipy.stats` documentation]<https://docs.scipy.org/doc/scipy/reference/stats.html>
2. [National Library of Medicine - Normality Tests]<https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3900058/>
3. [National Library of Medicine - Chi-square test]<https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3900058/> 
4. [scribbr.com - Kurtosis]<https://www.scribbr.com/statistics/kurtosis/>
5. [datatab.net - Mann-Whitney U-Test]<https://datatab.net/tutorial/mann-whitney-u-test>
