...
or
`pip3 install scipy.stats`
depending on what version of python you are using. 
...
variable are mutually exclusive, observations are independent, and both
...
$\sigma_{U} = \sqrt{\frac{n_{1}n_{2}}{12}((n + 1) - \frac{\sum_{k = 1}^{K}(t^{3}_{k} - t_{k})}{n(n - 1)})}$
::: 
